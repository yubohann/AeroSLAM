...
class TestPythonUUID(unittest.TestCase):
    ...
    def test_time(self):
        ...
        uu_time = rospy.Time()
        uu_time.secs = int(ns_intervals_epoch / 1e9 * 100)
        uu_time.nsecs = int(ns_intervals_epoch - uu_time.secs / (100 * 1e-9)) * 100
    ...
    def test_time_same_interval(self):
        ...
        ns_intervals_epoch = ns_intervals_rfc - offset
        uu1_time = rospy.Time()
        uu1_time.secs = int(ns_intervals_epoch / 1e9 * 100)
        uu1_time.nsecs = int(ns_intervals_epoch - uu1_time.secs / (100 * 1e-9)) * 100

        # Get time associated with uuid uu2
        ns_intervals_rfc = uu2.fields[0] + (uu2.fields[1] << 32) + ((uu2.fields[2] & 0x0FFF) << 48)
        ns_intervals_epoch = ns_intervals_rfc - offset
        uu2_time = rospy.Time()
        uu2_time.secs = int(ns_intervals_epoch / 1e9 * 100)
        uu2_time.nsecs = int(ns_intervals_epoch - uu2_time.secs / (100 * 1e-9)) * 100
