...
/**
 * @file AStar_Node.cpp
 * @brief Implementation of the Hybrid A* / A* node helpers.
...
namespace hlpmpccorridor_local_planner {  //定义带参数的构造函数的实现
...
bool AStar_Node::operator<(const AStar_Node &e) const { //定义小于操作符 < 的重载实现
    return cost > e.cost; //实现优先队列中的排序逻辑
}
